...
def run(board_count, modification, board_serial_number_list):
    ...
    def tcp_to_serial_bridge_restart(board_count):
        ...
        time.sleep(2)
        if not 'active (running)' in subprocess.run(['systemctl', 'status', f'tcp-to-serial-bridge-router{board_count}'],
        ...
        if 'неизвестная ошибка' in result[f'device_num_{dev_num}']['error']:
            ...
            if hdd_present or nmc_ports_count != 0:
                if ext_slot_out_result in ['Внешний HDD найден', 'Все порты NMC модуля найдены'] and \
                    ...
                    output_file.flush()
                    create_device_and_write_serial_num_pcb_and_reserve_macs(engine, serial_num_board)
